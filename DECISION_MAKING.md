...
4. [ICS 출력 — METHOD:PUBLISH + UID 멱등성 + UTC 일관 + DTSTAMP 안정](#4-ics-출력--methodpublish--uid-멱등성)
...
2. **UID 멱등성** — `naver:${gameId}@lck-schedule-sync` 형식. 같은 매치 = 항상 같은 UID → 캘린더가 in-place 갱신 (중복 없음), 사라진 UID는 자동 삭제.
...
**근거**: 이미 본 매치를 캘린더에서 다시 봐도 결과가 노출되지 않게. 팬 도구의 본질.

**한계**: 사용자가 결과를 보고 싶으면 다른 채널(OP.GG, 라이브 페이지)로.
