...
### 4.3 ~~의도적 손실 — 스포일러 회피~~ (2026-05-17 정책 전환 → §4.7)

**~~문제~~**: 네이버 응답에 결과(승자·점수·전적) 필드가 있음. 포함하면 정보 풍부.

**~~선택~~**: 결과 필드 의식적 미사용 (이미 본 매치 다시 봐도 결과 노출 X).

**전환 이유**: §4.7 참조. 단순 폐기가 아니라 **불포함 → 포함 정책 전환**. 본인 사용자 우선 가치("점수 보고 싶음") + 사용자 분포(거의 단독) 고려. 미래 사용자 늘면 옵트인 분리 가능.
...
**문제**: 초기엔 `DTSTART;TZID=Asia/Seoul:20260408T190000` + VTIMEZONE 블록 발행. UTC 시각에 +9h shift trick(KST 변환 헬퍼) 필요 + ICS에 VTIMEZONE 13줄 추가.
...
- public getter: `summary`·`description`·`location`·`url` (외부 노출)
- private helper: `stageText`·`bestOfText`·`scoreText`·`streamText`·`chzzkLiveUrl`·`naverVodUrl` — DESCRIPTION 슬롯별 한 줄씩
- `description` 게터가 status별로 라인 조립 — `ics.ts`는 형식화만 (SRP 유지)

> 본 섹션의 게터 이름은 §4.9 SUMMARY/DESC 재설계 이후 형태. 첫 도입 시점엔 `scoreLabel·bestOfLabel·streamUrl` 명명이었으나 이후 일관성 위해 `*Text` suffix로 통일.
