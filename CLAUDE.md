...
4. **시간 처리**: UTC로 일관 — 보관은 UTC ISO, ICS 출력은 RFC 5545 UTC compact (`Z` suffix). 캘린더 앱이 사용자 로컬 timezone으로 자동 변환 (한국 사용자 = KST 표시, 해외 한국 팬 = 현지 시각). VTIMEZONE 블록 불필요. 네이버 응답(epoch ms UTC)도 동일 정규화.
5. **동기화 신호 분리 (RFC 정합)**: `DTSTAMP`는 RFC §3.8.7.2 정의대로 직렬화 시각(매 발행 변동). 변경 감지의 진짜 신호는 `SEQUENCE`(§3.8.7.4) + `LAST-MODIFIED`(§3.8.7.3) — **콘텐츠 변경 시에만 갱신**. 이전 발행분 `X-CONTENT-HASH`를 읽어 매치별 diff. 같은 매치=같은 메타 보존(Outlook noise 0), 진짜 변경=정확한 갱신(in-place 갱신). UID 멱등성(`naver:gameId@lck-teams-schedule`)은 그대로 — 같은 매치는 항상 같은 UID. (Phase 4 후속 — "Phase 4 후속 2" 섹션 참조)
6. **개인화 위임**: VALARM 미포함. 알림·색·이름은 캘린더 앱에 위임 + README 시나리오 안내
7. **추상화는 두 번째 사례 등장 시**: 어댑터 인터페이스·소스 분기 X. 두 번째 데이터 소스가 실제로 필요해질 때(예: 네이버 영구 차단 시) 그때 합성 추출.
8. **ICS generator는 옵션 객체 받는 순수 함수**: 미래 확장 여지 보존
9. **소스 일관성 = 데이터 정확성**: ICS `METHOD:PUBLISH`로 발행된 구독 캘린더는 read-only(RFC 5546) → 이벤트별 메모·알림·색 설정 자체가 불가하므로 그 손실은 fallback 근거가 못 됨. 진짜 비용은 (a) fallback이 반쪽짜리라는 점 — lolesports는 6 대회 중 4 대회만 커버, KeSPA·EWC 매치는 fallback 발동 시 통째 사라짐, (b) UID 네임스페이스 전환 시 캘린더가 "전체 삭제 + 재추가"로 인식 → 시각적 flapping + 캘린더 단위 기본 알림이 같은 매치에 재발화 가능. 두 항목으로 충분히 트랩.
...
├── sync-meta.ts          # SEQUENCE/LAST-MODIFIED 결정 + 이전 ICS parser + contentHash (순수, Phase 4 후속 2)
...
## Phase 3 후속 2: DTSTAMP 안정화 (2026-05-17) — 2026-05-18 부분 폐기

> **갱신 (2026-05-18, Phase 4 후속 2)**: 본 결정 **부분 폐기**. SEQUENCE/LAST-MODIFIED를 도입하면서 DTSTAMP는 RFC §3.8.7.2 정의대로 `now` 환원. 본래 목표인 "Outlook 노이즈 0"은 SEQUENCE가 안정값(콘텐츠 동일하면 같은 값)이라 그대로 달성 — 수단이 더 정확해진 것. 자세한 흐름은 본문 끝 "Phase 4 후속 2" 섹션 참조.
...
## Phase 4 후속 2: 동기화 메타 RFC 정합 전환 (2026-05-18)

**결정**: ICS 동기화 신호를 RFC 5545 정합으로 재정렬. **DTSTAMP=now 환원** + **`SEQUENCE`(§3.8.7.4) + `LAST-MODIFIED`(§3.8.7.3) + `CREATED`(§3.8.7.1) 신규 도입**. 이전 발행분과 매치별 콘텐츠 hash diff로 변경 감지. Phase 3 후속 2 "DTSTAMP 안정화" 결정 부분 폐기 (수단 교체 — 본래 목표 "Outlook noise 0"은 SEQUENCE가 안정 신호 역할을 맡아 그대로 유지).

### 문제 (사용자 보고)

Google 캘린더 구독자가 ICS 재발행 후에도 매치 콘텐츠 변경(점수 추가·VOD URL 등)을 반영받지 못함. 진단:

- UID 멱등 ✅ + DTSTAMP 안정값 ✅ + SEQUENCE 부재 ❌ + LAST-MODIFIED 부재 ❌
- 클라이언트가 ICS를 fetch해도 "모든 메타 동일"로 판단 → 콘텐츠 diff 무시 → 사용자가 보는 캘린더 stale.
- 즉 Phase 3 후속 2의 "DTSTAMP 안정화"가 Outlook noise는 0으로 만들었으나 **진짜 변경 신호까지 0으로 만들었음** (의도치 않은 부작용).

### 해결 — 메타 역할 분리

| 메타           | RFC 정의                          | TO-BE                                        |
| -------------- | --------------------------------- | -------------------------------------------- |
| UID            | 영구 불변 식별자                  | `naver:{gameId}@lck-teams-schedule` 그대로   |
| DTSTAMP        | iCalendar 객체 직렬화 시각        | **`now` (매 발행 변동)** ← 환원              |
| CREATED        | 이벤트 첫 생성 시각               | `match.startDate` (deterministic, 영구 동일) |
| LAST-MODIFIED  | 콘텐츠 마지막 변경 시각           | **변경된 매치만 `now`, 미변경은 이전값**     |
| SEQUENCE       | 첫 발행 0, 의미 있는 변경마다 +1  | **변경된 매치만 +1, 미변경은 이전값**        |
| X-CONTENT-HASH | 비표준 X-property (§3.8.8.2 허용) | SHA-256(콘텐츠) — 다음 빌드 diff 복원용      |

### 변경 감지 알고리즘

빌드 시 GitHub Pages에서 직전 ICS HTTP GET → `parsePreviousIcs`로 UID별 `{sequence, contentHash, lastModified}` map 복원 → 매치별 새 hash 계산 후 비교:

- 신규 UID: `sequence=0, lastModified=now`
- hash 동일: 이전 sequence·lastModified 유지 (Outlook noise 0)
- hash 다름: `sequence+1, lastModified=now` → 클라이언트 in-place 갱신 trigger
- HTTP fetch 실패(404 / 네트워크 오류): cold start로 fall through (모든 매치 `sequence=0`)

별도 state 파일 불필요 — 자기 자신이 발행한 ICS가 곧 상태 저장소.
...
| 파일                             | 변경                                                                                                                                                |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/sync-meta.ts`               | **신규** — `parsePreviousIcs` · `computeContentHash` · `decideSyncMeta` + 타입 (순수 함수)                                                          |
| `src/ics.ts`                     | `IcsOptions`에 `now`·`veventMetaByUid` 추가, VCAL 헤더 3개 추가, VEVENT에 CREATED·LAST-MODIFIED·SEQUENCE·X-CONTENT-HASH 추가, `uidOf(match)` export |
| `src/main.ts`                    | `fetchPreviousIcs` + 매치별 sync meta 결정 → `generateIcs` 주입                                                                                     |
| `tests/unit/sync-meta.test.ts`   | **신규** — parser·hash·decideSyncMeta 단위 테스트                                                                                                   |
| `tests/unit/ics.test.ts`         | DTSTAMP=now 검증, SEQUENCE/LAST-MODIFIED/CREATED/X-CONTENT-HASH 검증, VCAL 신규 헤더 검증, 옛 DTSTAMP 안정성 테스트 폐기                            |
| `CLAUDE.md`·`DECISION_MAKING.md` | Phase 3 후속 2 부분 폐기 명시 + 본 섹션 신규                                                                                                        |

### 라이브러리 미도입 (ts-ics 등)

평가 결과 직접작성 유지가 더 적합:

- 정적 ICS 생성 라이브러리(ts-ics·ics)는 모두 **SEQUENCE 자동 관리 X** — 호출자가 결정 후 주입. DB 백엔드 없는 본 환경에선 `sync-meta.ts` 같은 로직이 어차피 필요 (라이브러리 도입으로 줄어들지 않음).
- ts-ics 도입 시 URL-aware folding(진행 중 PR에서 정착)을 post-process로 재구현해야 함 → 두 단계 직렬화로 어색.
- ICS parser도 자체 구현 ~30줄(정규식 기반 UID·SEQUENCE·LAST-MODIFIED·X-CONTENT-HASH 추출)로 충분.

### Trade-off (정직히)

- **GitHub Pages 캐시(`Cache-Control: max-age=600`) 사용자 제어 불가**: ICS 측에서 해결 못 함. 캘린더 클라이언트의 자체 throttling(Google 12-24h, Apple 1-3h)이 더 큰 변수이므로 실용적 영향 작음.
- **HTTP GET 실패 시 cold start**: 모든 매치 `sequence=0` 리셋. 매우 드물지만 발생 시 클라이언트 입장에서 "모든 매치 첫 게시처럼" 보일 수 있음 — UID 그대로라 in-place 갱신은 정상.
- **X-CONTENT-HASH 비표준**: 일부 strict parser가 무시할 수 있지만 본 프로젝트가 자기 자신 ICS만 read하므로 영향 0.
- **Phase 3 후속 2 부분 폐기**: 의사결정 진화 기록 명시. 본 변경의 본질은 _목표는 같고 수단이 더 정확해진 것_ — Outlook noise 0(원래 목표)은 SEQUENCE가 안정 신호 역할로 그대로 달성.

### 검증

- 172개 단위 테스트 통과 (sync-meta 신규 22개 포함)
- `pnpm check` 전체 통과 (typecheck + lint + format + test)

## Phase 4 후속 3: SUMMARY/DESC 재설계 (2026-05-18)

**결정**: SUMMARY를 `[SHORT_CODE] matchup`으로 좁히고, stage·BoN·결과·시청은 DESC 책임, 위치는 LOCATION 필드 책임으로 분리. 이모지 prefix로 DESC 모든 행에 시각 일관성.

### 결정 사항

| 위치                 | 책임                            | 예시                                     |
| -------------------- | ------------------------------- | ---------------------------------------- |
| SUMMARY              | 대회(short code) + 매치업       | `[LCK] T1 vs 젠지`                       |
| LOCATION 필드        | 경기장 (캘린더 앱 Maps 연동)    | `치지직 롤파크`                          |
| DESCRIPTION 🎯 행    | stage (네이버 raw)              | `🎯 플레이오프 2R`                       |
| DESCRIPTION 🎮 행    | BoN + 한국어 풀이               | `🎮 Bo5 (5판 3선승제)`                   |
| DESCRIPTION 🏆 행    | 경기 결과 (완료만)              | `🏆 경기 결과: 1 vs 3 (BNK 피어엑스 승)` |
| DESCRIPTION 📺/🎬 행 | 라이브 (예정) / 다시보기 (완료) | `🎬 다시보기: https://...`               |

### LEAGUE_SHORT_CODE (신규, 5자 이하 통일)

| League      | SHORT_CODE | LEAGUE_DISPLAY_NAME (유지) |
| ----------- | ---------- | -------------------------- |
| LCK         | `LCK`      | LCK                        |
| MSI         | `MSI`      | MSI                        |
| WORLDS      | `WORLDS`   | 월드 챔피언십              |
| FIRST_STAND | `FST`      | First Stand                |
| EWC         | `EWC`      | EWC                        |
| KESPA_CUP   | `KESPA`    | KeSPA Cup                  |

`LEAGUE_DISPLAY_NAME`은 도메인 표준 한국어명으로 유지 (landing page·다른 용도).

### 근거

1. **SUMMARY는 한 줄 식별 본질**: 매치업 + 대회만 — stage·BoN은 펼침 시 부가 정보
2. **모바일 그리드 잘림 회피**: 한국어 LEAGUE_DISPLAY_NAME(`월드 챔피언십` 9자·`First Stand` 13자·`KeSPA Cup` 11자)이 SUMMARY에 들어가면 잘릴 위험. 영문 SHORT_CODE 5자 이하 통일
3. **DESC 이모지 일관성**: 일부 행만 이모지였던 비일관성 → 모든 행 이모지 prefix
4. **SRP 명확화**: LOCATION은 LOCATION 필드만 책임. DESC `📍` 제거 (Google·Apple·Outlook 모두 LOCATION 필드 자체 표시)
5. **matchup·tournament DESC 중복 제거**: SUMMARY와 1:1 중복이라 펼침 시 노이즈

### 미세 결정

- **EWC 시각적 중복 허용**: 네이버 raw title `Road to EWC X`가 league명 포함 → SUMMARY `[EWC]` + DESC `🎯 Road to EWC X`. 한 번뿐인 중복, raw 표기 보존이 정보 우선
- **stage 부재 시 🎯 행 생략**: WORLDS 같이 raw stage 없을 수도 있는 케이스 안전

### 변경 면적

| 파일                                     | 변경                                                                                                               |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `src/league.ts`                          | `LEAGUE_SHORT_CODE` 신규                                                                                           |
| `src/match.ts`                           | summary·description 재설계, `tournamentLabel()` 제거, `stageText()` 신규, `bestOfText()`·`scoreText()` 이모지 추가 |
| `tests/unit/ics.test.ts`·`match.test.ts` | SUMMARY/DESC 형식 검증 일제 갱신                                                                                   |

### 첫 적용 SEQUENCE 영향

SUMMARY·DESCRIPTION 변경 = `X-CONTENT-HASH` 변경 → 모든 매치 `SEQUENCE +1` 1회 일제 발생 (Phase 4 후속 2의 자연 동작). 사용자 캘린더에서 모든 매치 "수정됨" 1회 표시 후 안정. 정상 동작.

### 검증

- 172개 단위 테스트 통과
- `pnpm dev` 실제 발행 결과 첫 VEVENT 확인 — 새 형식 정확히 출력 (`SUMMARY:[LCK] T1 vs BNK 피어엑스` · `DESCRIPTION:🎯 플레이오프 2R\n🎮 Bo5 (5판 3선승제)\n🏆 경기 결과: ...\n🎬 다시보기: ...`)
